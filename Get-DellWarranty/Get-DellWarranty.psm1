Function Global:Get-DellWarranty {
    ...
    if ($Api -ne $true){
        ...
        if ((Test-Path "$env:public\Dell\DellSec.txt")-ne $true){
            ...
            $ApiKey = Get-Content "$env:public\Dell\DellKey.txt"
            $ApiSecret = Get-Content "$env:public\Dell\DellSec.txt"
        ...
        Read-Host -Prompt "`nPlease provide API Key" | Out-File $env:public\Dell\DellKey.txt -Force
        Read-Host -Prompt "`nNow please provide API Secret" | Out-File $env:public\Dell\DellSec.txt -Force
